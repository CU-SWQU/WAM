...
!
       ...
       use idea_solar,       only  : nwaves,NWAVES_EUV,lyman_a_num,nwaves_src
       use idea_solar,       only  : sfmin, afac,rlmeuv
! 
      use wam_date_calendar, only : COPY_IDAT_NEMS_2_WAM, idat_wam, irhour_wam
...
! SRB heating by O2 (don't need efficiencies if self consistently
...
!
! Below we read data on each PE separately, namelists/global F107/Kp/NO 
!
! should be done on the designated IO-PE and MPI_BCASTED => to all PEs
!
! this was done on  Zeus, but initial issues with mpif.h on Theia
! enforced to read on all PEs.....
!
!======================================================================

      call MPI_COMM_RANK (MPI_COMM_WORLD, mpi_id, mpi_err)
      CALL solar_read_namelist(nml_solar, nlun_solar, ncfile_fpath, noeof_file,  mpi_id)
!
!SNOE-NO
!
...
! (17) => (levs)
!
      CALL  interpol_wamz( nz_euv, z17, effuv17,  levs, Z, effuv,  kup,kdw)
      CALL  interpol_wamz( nz_euv, z17, effeuv17, levs, Z, effeuv, kup,kdw) 
